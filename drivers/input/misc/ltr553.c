...
static int sensor_pinctrl_init(struct device *dev, struct pinctrl_config *config)
{
	...
	config->state[0] = pinctrl_lookup_state(config->pinctrl, config->name[0]);
	if (IS_ERR_OR_NULL(config->state[0])) {
		dev_err(dev, "Failed to look up %s\n", config->name[0]);
		return PTR_ERR(config->state[0]);
	}

	config->state[1] = pinctrl_lookup_state(config->pinctrl, config->name[1]);
...
static int ltr553_parse_dt(struct device *dev, struct ltr553_data *ltr)
{
	...
	rc = of_get_named_gpio_flags(dp, "liteon,irq-gpio", 0, &ltr->irq_flags);
	if (rc < 0) {
	...
	/* ps distance table */
	rc = of_property_read_u32_array(dp, "liteon,ps-distance-table", ps_distance_table, ARRAY_SIZE(ps_distance_table));
	...
	/* als equation map */
	rc = of_property_read_u32_array(dp, "liteon,als-equation-0", &eqtn_map[0].ch0_coeff_i, 6);
	if (rc)
		dev_warn(dev, "read liteon,als-equation-0 failed. Drop to default\n");

	rc = of_property_read_u32_array(dp, "liteon,als-equation-0", &eqtn_map[1].ch0_coeff_i, 6);
	if (rc)
		dev_warn(dev, "read liteon,als-equation-1 failed. Drop to default\n");

	rc = of_property_read_u32_array(dp, "liteon,als-equation-0", &eqtn_map[2].ch0_coeff_i, 6);
	if (rc)
		dev_warn(dev, "read liteon,als-equation-2 failed. Drop to default\n");

	rc = of_property_read_u32_array(dp, "liteon,als-equation-3", &eqtn_map[3].ch0_coeff_i, 6);
	if (rc)
...
static int ltr553_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		if (res) {
			dev_err(&client->dev, "unable to parse device tree.(%d)\n", res);
			goto out;
		}
	} else {
	...
	if (gpio_is_valid(ltr->irq_gpio)) {
		...
		res = devm_request_irq(&client->dev, ltr->irq, ltr553_irq_handler, ltr->irq_flags | IRQF_ONESHOT, "ltr553", ltr);
